...
void logBoot(const char* moduleName, bool isOk) {
    // CHECK 3.1: Imprimir el nombre del módulo en la posición actual del cursor:
    // display.print(moduleName);
    display.print(moduleName);

    // CHECK 3.2: Alinear el estado a la derecha en la columna X=95 sin alterar la fila Y actual.
    ...
    // CHECK 3.3: Según el valor de isOk (true/false), imprimir "[OK]" o "[ERR]":
    // if (isOk) {
    //     display.println("[OK]");
    // } else {
    //     display.println("[ERR]");
    ...
    // CHECK 3.4: Volcar los cambios del buffer al vidrio físico y pausar 200ms para efecto visual:
    // display.display();
    // delay(200);
    display.display();
    delay(200);
...
void setup() {
    ...
    // CHECK 3.5: Probar la función logBoot() con dos módulos de prueba:
