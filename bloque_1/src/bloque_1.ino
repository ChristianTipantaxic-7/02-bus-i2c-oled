...
void setup() {
    ...
    // CHECK 1.1: Inicializar el bus I2C en los pines SDA y SCL del ESP32.
    // Pregunta Guía: ¿Qué función de la librería Wire recibe (SDA_PIN, SCL_PIN)?
    // Pista: Usa Wire.begin(I2C_SDA_PIN, I2C_SCL_PIN);
    Wire.begin(I2C_SDA_PIN, I2C_SCL_PIN);

    // CHECK 1.2: Configurar la velocidad del reloj a 400kHz (Modo Rápido).
    // Pregunta Guía: ¿Qué función configura la frecuencia de reloj del bus I2C?
    // Pista: Usa Wire.setClock(I2C_CLOCK_SPEED);
    Wire.setClock(I2C_CLOCK_SPEED);

    Serial.println("[I2C] Bus configurado en SDA:GPIO21, SCL en GPIO22 a 400kHz.\n");
    Serial.println("--- INICIANDO BARRIDO DE DIRECCIONES (0x01 .. 0x7E) ---");

    int devicesFound = 0;

    // CHECK 1.3: Recorrer las direcciones válidas de 7 bits (del 1 al 126).
    for (byte address = 1; address < 127; address++) {

        // CHECK 1.4: Tocar la puerta del periférico:
        ...
        // CHECK 1.5: Evaluar la respuesta del periférico:
