...
void runSystemPOST() {
    ...
    // CHECK 4.1: Mostrar la cabecera visual invocando showBootHeader()
    showBootHeader();

    // CHECK 4.2: Probar secuencialmente los 4 subsistemas usando logBoot(nombre, estado):
    ...
    // CHECK 4.3: Concluir la rutina mostrando la barra final con showSystemReady()
    showSystemReady();
}
...
void setup() {
    ...
    // CHECK 4.4: Orquestar el arranque:
    ...
    if (total > 0 && initDisplay()) {
        ...
        Serial.println("[BOOT] ERROR: No se pudo completar el arranque del sistema.");
    }
}
